...
void simulation::perform_smmc_move(double * coords)
{
    ...
    double backbone_disp[3],backbone_q[4],backbone_q_conj[4],backbone_rmsd;
#ifdef DEBUG
    FILE * output;
    ...
    do {
        ipose=int(smmc_nposes*genrand_real3());
    ...
    rmsd_fit(top->natom,wt_backbone,&smmc_structures[3*top->natom*ipose],coords,&backbone_disp[0],&backbone_q[0],&backbone_rmsd);
    ...
    rotate_atoms_by_point(top->ligand,&q1[0],&ligand_com[0],coords);
    //translate by disp1
