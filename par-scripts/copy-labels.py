...
pdbwithnames=sys.argv[2]
drug=sys.argv[3]
newpdb=sys.argv[4]
equivfname=sys.argv[5]
...
for line in infile:
        ...
        if ((line[0:6]=='ATOM  ') or (line[0:6]=='HETATM')):
                #it's a set of atomic coordinates:
                index=int(line[6:11])
                refgraph.add_node(index)
                name=line[12:16].lstrip().rstrip()
                x=float(line[30:38])
                y=float(line[38:46])
                z=float(line[46:54])
                #print el, x, y, z
                names[index] = name
                refcoords[index] = np.array([x, y, z])

infile.close()

#dmatrix=[]
#The reference does not contain bonding info, so we have to determine it.  
#The distance cutoff (1.7 A) is not uniform for all drugs.
for i in refgraph.nodes():
	for j in refgraph.nodes():
		if (j>i):
			v=refcoords[j]-refcoords[i]
			d=math.sqrt(np.dot(v,v))
			print i, j, d
			if ((d<1.6) and (refgraph.degree(i)<4) and (refgraph.degree(j)<4)):
 				refgraph.add_edge(i,j)
			#dmatrix.append((i, j, d))

#print sdfelement
#print sdfcoords
print sdfgraph.number_of_edges()
print sdfgraph_noh.number_of_edges()
print refgraph.number_of_edges()
print names
...
equivcounter=1
#First match the two molecule graphs to see if the PDB has hydrogens. 
pdb_has_hydrogens=False
matcher=isomorphism.GraphMatcher(sdfgraph,refgraph)
ok=matcher.is_isomorphic()
if (ok):
	pdb_has_hydrogens=True
else:
	matcher=isomorphism.GraphMatcher(sdfgraph_noh,refgraph)
	ok=matcher.is_isomorphic()
